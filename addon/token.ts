...
const expandOnce = (
...
): string => data.replace(/([^\x7F]?)\0(\d+)g\x7F/gu, (_, prev: string, i: string) => {
	...
	if (target.type === 'translate') {
		...
		return prev
			+ lastChild.firstChild!.toString()
				.replace(/^\n|\n$/gu, '')
				.replace(
					/\0(\d+)c\x7F[\n ]/gu,
					(m, p: string) => {
						const {innerText} = accum[p as unknown as number] as CommentToken;
						return /^T:[^_/\n<>~]+$/u.test(innerText) ? '' : m;
					},
				)
				.replace(/\0\d+n\x7F/gu, '');
	}
	// @ts-expect-error sparse array
	accum[accum.indexOf(target)] = undefined;
	return prev + removeCommentLine(target.innerText);
	...
	if (name === 'if' && (known || knownVar1) || known && (name === 'ifexist' || name === 'iferror')) {
		...
		if (j === length) { // 不含复杂参数
			...
			return expandOnce(
				implicitNewLine(defaultVal, prev),
				config,
				include,
				now,
				accum,
...
const expand = (
	...
	for (const plain of [...accum.slice(n), token]) {
		...
		const expanded = expandOnce(data, config, include, now, accum, stack, page, callPage, context);
