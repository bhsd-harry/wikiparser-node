...
`))},Ka=function(){if(w(this,s).excludes.includes("extLink"))return;const{parseExternalLinks:m}=(Fa(),ee(La));this.setText(m(this.firstChild.toString(),w(this,s),w(this,i)))},Ya=function(){if(w(this,s).excludes.includes("magicLink"))return;const{parseMagicLinks:m}=(Zu(),ee(Ra));this.setText(m(this.firstChild.toString(),w(this,s),w(this,i)))},Za=function(){if(w(this,s).excludes.includes("list"))return;const{parseList:m}=(sd(),ee(qa)),{firstChild:y,type:S,name:A}=this,L=y.toString().split(`
`),C={lastPrefix:""};let F=S==="root"||S==="ext-inner"&&A==="poem"?0:1;for(;F<L.length;F++)L[F]=m(L[F],C,w(this,s),w(this,i));this.setText(L.join(`
...
`:"")+C.description},range:O!=null?O:Ue(b)}}async provideSignatureHelp(g,h){if(!this.data)return;const{line:x,character:b}=h,m=g.split(/\r?\n/u,x+1)[x],{lastChild:y}=await R(this,u,Cn).call(this,`${m.slice(0,b+/^[^{}<]*/u.exec(m.slice(b))[0].length)}}}`);if(!y.is("magic-word")||y.length===1)return;const{name:S,childNodes:A,firstChild:L}=y,C=R(this,u,bt).call(this,S);if(!(C!=null&&C.signatures))return;const F=A.length-1,B=C.signatures.filter(W=>{var q;return(W.length>=F||((q=W[W.length-1])==null?void 0:q.rest))&&W.every(({label:z,const:M},Q)=>{var se;const J=M&&Q<F&&((se=A[Q+1])==null?void 0:se.text().trim());return!J||z.toLowerCase().includes(J.toLowerCase())})});if(B.length===0)return;let O=-1;for(let W=y.getAbsoluteIndex()+y.getAttribute("padding");O<F&&(W+=A[O+1].toString().length+1,!(W>b));O++);const E=L.text().trim(),D=y.getAttribute("colon");return{signatures:B.map(W=>({label:`{{${E}${W.length===0?"":D}${W.map(({label:q})=>q).join("|")}}}`,parameters:W.map(({label:q,const:z})=>({label:q,...z?{documentation:"Predefined parameter"}:void 0})),...W.length<F?{activeParameter:Math.min(O,W.length-1)}:void 0})),activeParameter:O}}async provideInlayHints(g){const h=await R(this,u,Fe).call(this,g);let x=[];for(const b of h.querySelectorAll("template,magic-word#invoke").reverse()){const{type:m,childNodes:y}=b;x=[...x,...y.slice(m==="template"?1:3).filter(({anon:S})=>S).reverse().map(S=>({position:mt(h,S.getAbsoluteIndex()),label:`${S.name}=`,kind:2}))]}return x}findStyleTokens(){return w(this,l).querySelectorAll(ao).filter(({lastChild:g})=>Os(g))}async provideRefactoringAction(g,h){let x,b;if(h){const{start:S,end:A}=h;if(S.line===A.line&&S.character===A.character)return[];x=g.split(/\r?\n/u,A.line+1),b=S.line===A.line?x[A.line].slice(S.character,A.character):`${x[S.line].slice(S.character)}
${x.slice(S.line+1,A.line).join(`
`)}${x.length===2?"":`
`}${x[A.line].slice(0,A.character)}`}else if(g)x=g.split(/\r?\n/u),b=x.join(`
`),h={start:{line:0,character:0},end:{line:x.length-1,character:x[x.length-1].length}};else return[];const m=await R(this,u,Cn).call(this,b),{viewOnly:y}=N;return N.viewOnly=!1,m.escape(),N.viewOnly=y,[{title:"Escape with magic words",kind:"refactor.rewrite",edit:{changes:{"":[{range:h,newText:m.toString()}]}}}]}},e=new WeakMap,t=new WeakMap,s=new WeakMap,i=new WeakMap,l=new WeakMap,n=new WeakMap,r=new WeakMap,o=new WeakMap,a=new WeakMap,u=new WeakSet,rs=function(){return w(this,r)===this.config&&w(this,o)===this.include},Fe=async function(g){var h;return g=us(g),!w(this,s)&&R(this,u,rs).call(this)&&w(this,e)===g?w(this,l):(j(this,e,g),(h=w(this,s))!=null||j(this,s,R(this,u,Tn).call(this)),w(this,s))},Tn=async function(){var x;(x=this.config)!=null||(this.config=N.getConfig()),j(this,r,this.config),j(this,o,this.include);const g=w(this,e),h=await N.partialParse(g,()=>w(this,e),this.include,this.config);return R(this,u,rs).call(this)&&w(this,e)===g?(j(this,l,h),j(this,s,void 0),h):(j(this,s,R(this,u,Tn).call(this)),w(this,s))},Cn=async function(g){var h;return g=us(g),!w(this,i)&&R(this,u,rs).call(this)&&w(this,t)===g?w(this,n):(j(this,t,g),(h=w(this,i))!=null||j(this,i,R(this,u,In).call(this)),w(this,i))},In=async function(){var x;(x=this.config)!=null||(this.config=N.getConfig()),j(this,r,this.config),j(this,o,this.include);const g=w(this,t),h=await N.partialParse(g,()=>w(this,t),this.include,this.config);return R(this,u,rs).call(this)&&w(this,t)===g?(j(this,n,h),j(this,i,void 0),h):(j(this,i,R(this,u,In).call(this)),w(this,i))},ru=function(){var g;if(!w(this,a)||w(this,a)[1]!==this.config){(g=this.config)!=null||(this.config=N.getConfig());const{nsid:h,ext:x,html:b,parserFunction:[m,y,...S],doubleUnderscore:A,protocol:L,img:C}=this.config,F=new Set([x,b].flat(2)),B=new RegExp("(?:"+String.raw`<(\/?\w*)`+"|"+String.raw`(\{{2,4}|\[\[)\s*([^|{}<>[\]\s][^|{}<>[\]#]*)?`+"|"+String.raw`(__(?:(?!__)[\p{L}\p{N}_])*)`+"|"+String.raw`(?<!\[)\[([a-z:/]*)`+"|"+String.raw`\[\[\s*(?:${Object.entries(h).filter(([,O])=>O===6).map(([O])=>O).join("|")})\s*:[^[\]{}<>]+\|([^[\]{}<>|=]*)`+"|"+String.raw`<(\w+)(?:\s(?:[^<>{}|=\s]+(?:\s*=\s*(?:[^\s"']\S*|(["']).*?\8))?(?=\s))*)?\s(\w*)`+")$","iu");j(this,a,[{re:B,ext:x,tags:F,allTags:[...F,"onlyinclude","includeonly","noinclude"],functions:[Object.keys(m),Array.isArray(y)?y:Object.keys(y),S].flat(2),switches:A.slice(0,2).flat().map(O=>`__${O}__`),protocols:L.split("|"),params:Object.keys(C).filter(O=>O.endsWith("$1")||!O.includes("$1")).map(O=>O.replace(/\$1$/u,""))},this.config])}return w(this,a)[0]},Nn=function(g){return this.data.behaviorSwitches.find(({aliases:h})=>h.includes(g))},bt=function(g){return this.data.parserFunctions.find(({aliases:h})=>h.some(x=>x.replace(/^#/u,"")===g))},T)}}),lu={};ae(lu,{default:()=>N});ou.exports=ee(lu);var Ns,un,go,ze,dn,po,N,G=I({"index.ts"(){jn(),Oe(),Je(),fe(),od(),Ns=!0,un=new $n,ze={config:hn,get rules(){return as},get i18n(){return{...So,...go}},set i18n(e){go=e},get lintConfig(){return un},set lintConfig(e){un=new $n(e)},get viewOnly(){return Ns},set viewOnly(e){Ns&&!e&&we.rev++,Ns=e},getConfig(e){const t=e!=null?e:this.config,{doubleUnderscore:s,ext:i,parserFunction:l,variable:n}=t;for(let r=0;r<2;r++)s.length>r+2&&s[r].length===0&&(s[r]=Object.keys(s[r+2]));return i.includes("translate")&&!n.includes("translationlanguage")&&(n.push("translationlanguage"),Array.isArray(l[1])?l[1].push("TRANSLATIONLANGUAGE"):l[1].TRANSLATIONLANGUAGE="translationlanguage"),{...hn,...t,excludes:[]}},msg(e,t=""){var s;return e&&((s=this.i18n[e])!=null?s:e).replace("$1",this.msg(t))},normalizeTitle(e,t=0,s,i=ze.getConfig(),l){const{Title:n}=(ad(),ee(tu));let r;if(l!=null&&l.halfParsed)r=new n(e,t,i,l);else{const{Token:o}=(V(),ee(os));r=we.run(()=>{const a=new o(e,i);a.type="root",a.parseOnce(0,s).parseOnce();const u=new n(a.toString(),t,i,l);a.build();for(const d of["main","fragment"]){const c=u[d];if(c!=null&&c.includes("\0")){const f=a.buildFromStr(c,1);d==="main"?u.main=f:u.setFragment(f)}}return u})}return r},parse(e,t,s=ke,i=ze.getConfig()){e=us(e);let l;typeof s!="number"&&(l=Array.isArray(s)?s:[s],s=Math.max(...l.map(o=>wo[o]||ke)));const{Token:n}=(V(),ee(os));return we.run(()=>{const o=new n(e,i);return o.type="root",o.parse(s,t)})},async partialParse(e,t,s,i=ze.getConfig()){{const{Token:l}=(V(),ee(os)),n=typeof setImmediate=="function"?setImmediate:setTimeout,{running:r}=we;we.running=!0;const o=new l(us(e),i);o.type="root";let a=0;try{await new Promise(u=>{const d=()=>{t()===e?(a++,n(c,0)):u()},c=()=>{a===ke+1?(o.afterBuild(),u()):(o[a===ke?"build":"parseOnce"](a,s),d())};n(c,0)})}catch(u){throw we.running=r,u}return we.running=r,o}},createLanguageService(e={}){var t;{const s=(ud(),ee(iu)),{LanguageService:i,tasks:l}=s;return(t=l.get(e))!=null?t:new i(e)}throw new Error("Parser.createLanguageService method is only available in the LSP version!")}},dn={},po=new Set(["lintConfig","normalizeTitle","parse","createLanguageService"]);for(const e in ze)po.has(e)||(dn[e]={enumerable:!1});Object.defineProperties(ze,dn),Object.assign(typeof globalThis=="object"?globalThis:self,{Parser:ze}),N=ze}});G()});dd();})();
