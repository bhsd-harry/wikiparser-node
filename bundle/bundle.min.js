...
${a.shift()}`;const c=(x,m)=>{if(!m){d+=x;return}const{lastChild:A}=m;if(A.constructor===S)A.setText(String(A)+x);else{const F=new S(x,o,!0,l);F.type="table-inter",m.insertAt(F.setAttribute("stage",3))}};for(const x of a){let m=h.pop();const[A]=/^(?:\s|\0\d+c\x7F)*/u.exec(x),F=x.slice(A.length),C=/^(:*)((?:\s|\0\d+c\x7F)*)(\{\||\{\0\d+!\x7F|\0\d+\{\x7F)(.*)$/u.exec(F);if(C){for(;m&&m.type!=="td";)m=h.pop();const[,I,D,O,X]=C;I&&new r(I,o,l),c(`
