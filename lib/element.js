...
class AstElement extends AstNode {
	...
	static lintIgnoredHidden = new Set(['noinclude', 'include', 'comment', 'double-underscore', 'hidden']);
	 ...
	 */
	lint() {
		return AstElement.lintIgnoredHidden.has(this.type) || AstElement.lintIgnoredSyntax.has(this.type)
			|| this.type === 'ext-inner' && AstElement.lintIgnoredExt.has(this.name)
			? []
			: this.childNodes.flatMap(child => child.lint());
	}
