...
/** @typedef {import('./element').SelectorArray} SelectorArray */
/**
 * @template {string} T
 * @typedef {import('./node').TokenAttribute<T>} TokenAttribute<T>
 */
/** @typedef {import('./title')} Title */
...
/**
 * 检测:lang()伪选择器
 * @param {Token & {attributes?: Record<string, string|true>}} node 节点
...
class AstElement extends AstNode {
	 ...
	 */
	#matchesAttr = (key, equal = undefined, val = undefined, i = undefined) => {
		if (!equal) {
		...
		if (/** @type {unknown} */ (thisVal) instanceof RegExp) {
	...
	get children() {
		// eslint-disable-next-line no-extra-parens
		const children = /** @type {Token[]} */ (this.childNodes.filter(({type}) => type !== 'text'));
		return children;
	}
	...
	get firstElementChild() {
		// eslint-disable-next-line no-extra-parens
		return /** @type {Token} */ (this.childNodes.find(({type}) => type !== 'text'));
	}
	...
	getAttribute(key) {
		// eslint-disable-next-line no-extra-parens
		return key === 'matchesAttr' ? /** @type {TokenAttribute<T>} */ (this.#matchesAttr) : super.getAttribute(key);
	}
	...
	#isProtected() {
		...
		// eslint-disable-next-line no-extra-parens
		const {childNodes, constructor: {fixed}} = /** @type {Token & {constructor: {fixed?: boolean}}} */ (parentNode),
	 ...
	 * @this {this & Token & {link?: string|Title, constructor: {fixed?: boolean}}}
	...
	#matches(step) {
		...
		return step.every(selector => {
			if (typeof selector === 'string') {
				...
				// eslint-disable-next-line no-extra-parens
				return this.getAttribute('matchesAttr')(.../** @type {[string, string, string, string]} */(selector));
			}
			...
			switch (pseudo) {
				...
				case 'lang': {
					...
					return matchesLang(this, regex)
						|| this.getAncestors().some(ancestor => matchesLang(
							// eslint-disable-next-line no-extra-parens
							/** @type {Token & {attributes?: Record<string, string|true>}} */ (ancestor),
							regex,
						));
				}
				default:
	...
	matches(selector) {
		if (selector === undefined) {
			...
			const stack = parseSelector(selector),
				pseudos = new Set(
					// eslint-disable-next-line no-extra-parens
					/** @type {string[]} */ (stack.flat(2).filter(step => typeof step === 'string' && step[0] === ':')),
				);
			if (pseudos.size > 0) {
	...
	/**
	 * 以HTML格式打印
	 * @param {import('./element').printOpt} opt 选项
	 * @returns {string}
	 */
	print(opt = {}) {
		return String(this)
