...
class AstElement extends AstNode {
	...
	indexFromPos(top, left) {
		if (typeof top !== 'number' || typeof left !== 'number') {
			typeError('Number');
		} else if (top < 0 || !Number.isInteger(top) || left < 0 || !Number.isInteger(left)) {
			return;
		}
	...
	getIndex(j) {
		...
		let /** @type {(string|AstElement)[]} */ childNodes;
		/**
		 * 使用前需要先给`childNodes`赋值
		 * @param {number} end
		 * @param {AstElement} parent
		 */
		const getIndex = (end, parent) => childNodes.slice(0, end)
			.reduce((acc, cur, i) => acc + String(cur).length + parent.getGaps(i), 0)
			+ parent.getPadding();
		if (j === undefined) {
		...
		({childNodes} = this);
		if (j < 0 || j > childNodes.length || !Number.isInteger(j)) {
			throw new RangeError(`不存在第 ${j} 个子节点！`);
		}
	...
	getAbsoluteIndex() {
		const {parentElement} = this;
		return parentElement ? parentElement.getAbsoluteIndex() + this.getIndex() : 0;
	}
	...
	getPosition(j) {
		if (j === undefined) {
			...
			return parentElement.posFromIndex(this.getIndex());
		}
		return this.posFromIndex(this.getIndex(j));
	}

	getBoundingClientRect() {
		const /** @type {AstElement} */ root = this.getRootNode();
	 ...
	 */
	plain() {
		return this.childNodes.flatMap(/** @param {string|AstElement} node */ (node, i) => {
			if (node instanceof AstElement) {
				return node.plain();
			}
			return node ? [[this.getAbsoluteIndex() + this.getIndex(i), node]] : [];
		});
	}
}
