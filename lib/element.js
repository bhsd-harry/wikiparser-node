...
class AstElement extends AstNode {
	...
	get nextVisibleSibling() {
		...
		while (nextSibling === '' || typeof nextSibling !== 'string' && nextSibling.hidden) {
	...
	get previousVisibleSibling() {
		...
		while (previousSibling === '' || typeof previousSibling !== 'string' && previousSibling.hidden) {
	...
	static #simplePseudoRegex = new RegExp(`:(?:${this.#pseudoKeys.join('|')})(?:\\(.*?\\))?(?=:|\\s*(?:,|$))`, 'gu');
	...
	matches(selector = '', simple = false) {
		...
		const /** @type {pseudoCall} */ calls = Object.fromEntries(AstElement.#pseudoKeys.map(f => [f, []])),
			...
			plainPseudo = AstElement.#pseudoKeys.filter(f => !AstElement.#pseudo[f] && calls[f].length > 0);
	 ...
	 */
	get eof() {
		return this.type === 'root' || !this.nextSibling && this.parentNode.eof;
	}
