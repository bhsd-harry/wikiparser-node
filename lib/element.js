...
class AstElement extends AstNode {
	...
	getPosition() {
		...
		const {childNodes} = parentElement,
			index = childNodes.slice(0, childNodes.indexOf(this))
				.reduce((acc, cur, i) => acc + String(cur).length + parentElement.getGaps(i), 0);
		return parentElement.posFromIndex(index + parentElement.getPadding());
	}

	/** @returns {AstRect} */
	getBoundingClientRect() {
		const {parentElement} = this,
			rect = this.#getDimension();
		if (!parentElement) {
			return {...rect, top: 0, left: 0};
		}
		const {top, left} = parentElement.getBoundingClientRect(),
			{top: offsetTop, left: offsetLeft} = this.getPosition();
