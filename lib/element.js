...
class AstElement extends AstNode {
	...
	/**
	 * @param {AstPosition|number} pos
	 * @param {number} left
	 */
	indexFromPos(pos, left) {
		const top = typeof pos === 'object' ? pos.top : pos;
		left = typeof pos === 'object' ? pos.left : left;
	...
	get position() {
		...
		const {childNodes} = parentElement,
			index = childNodes.slice(0, childNodes.indexOf(this)).reduce((acc, cur) => acc + String(cur).length, 0);
		return parentElement.posFromIndex(index);
	}
	...
	getBoundingClientRect() {
		const {parentElement} = this,
			rect = this.#dimension;
		if (!parentElement) {
			return {...rect, top: 0, left: 0};
		}
		const {top, left} = parentElement.getBoundingClientRect(),
			{top: offsetTop, left: offsetLeft} = this.position;
		return {...rect, top: top + offsetTop, left: (offsetTop > 0 ? 0 : left) + offsetLeft};
	}
