...
class AstNode {
	...
	removeAt(i) {
		...
		if (i < -length || i >= length || !Number.isInteger(i)) {
	...
	/** @param {string} str */
	setText(str, i = 0) {
		if (typeof str !== 'string' || typeof i !== 'number') {
			typeError(this, 'setText', 'String', 'Number');
		} else if (i < 0 || i >= this.childNodes.length || !Number.isInteger(i)) {
			throw new RangeError(`不存在第 ${i} 个子节点！`);
		}
