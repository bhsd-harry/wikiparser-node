...
class AstNode {
	...
	/** @type {string|AstNode} */ firstChild = null;
	/** @type {string|AstNode} */ lastChild = null;
	/** @type {string|AstNode} */ nextSibling = null;
	/** @type {string|AstNode} */ previousSibling = null;
	/** @type {AstNode} */ parentNode = null;
	...
	constructor() {
		this.seal(['firstChild', 'lastChild', 'nextSibling', 'previousSibling', 'parentNode']);
		this.seal(['childNodes'], true);
	 ...
	 */
	getAttribute(key) {
		return this.hasAttribute(key) ? String(this[key]) : null;
	}
	...
	setAttribute(key, value) {
		if (this.hasAttribute(key)) {
			...
			if (!descriptor.writable && externalUse('setAttribute')) {
				throw new RangeError(`禁止手动指定 ${key} 属性！`);
			}
			if (value === undefined
				&& ['firstChild', 'lastChild', 'nextSibling', 'previousSibling', 'parentNode'].includes(key)
			) {
				value = null;
			}
	...
	#updateChildNodes(childNodes) {
		this.setAttribute('childNodes', childNodes)
			.setAttribute('firstChild', childNodes[0]).setAttribute('lastChild', childNodes.at(-1));
	}
	...
	removeAt(i) {
		...
		if (node instanceof AstNode) {
			node.setAttribute('parentNode').setAttribute('nextSibling').setAttribute('previousSibling');
		}
		this.#updateChildNodes(childNodes);
		const previousSibling = childNodes[i - 1],
			nextSibling = childNodes[i];
		if (previousSibling instanceof AstNode) {
			previousSibling.setAttribute('nextSibling', nextSibling);
		}
		if (nextSibling instanceof AstNode) {
			nextSibling.setAttribute('previousSibling', previousSibling);
		}
		return node;
	}
	...
	insertAt(node, i = this.childNodes.length) {
		...
		if (previousSibling instanceof AstNode) {
			previousSibling.setAttribute('nextSibling', node);
		}
		if (nextSibling instanceof AstNode) {
			nextSibling.setAttribute('previousSibling', node);
		}
		if (node instanceof AstNode) {
			node.parentNode?.removeChild(node);
			node.setAttribute('parentNode', this)
				.setAttribute('nextSibling', nextSibling).setAttribute('previousSibling', previousSibling);
		}
		return node;
	}
	...
	/**
	 * @param {string|AstNode} child
	 * @param {string|AstNode|null} reference
	 */
	...
	normalize() {
		...
		for (const [i, text] of [...childNodes.entries()].reverse()) {
			...
			if (text.toString() === '') {
				...
				if (previousSibling instanceof AstNode) {
					previousSibling.setAttribute('nextSibling', childNodes[i]);
				}
			} else if (typeof previousSibling === 'string') {
				childNodes[i - 1] += text; // 不能用previousSibling替代childNodes[i - 1]
				childNodes.splice(i, 1);
			} else {
				continue;
			}
			const nextSibling = childNodes[i];
			if (nextSibling instanceof AstNode) {
				// 不能用previousSibling替代childNodes[i - 1]
				nextSibling.setAttribute('previousSibling', childNodes[i - 1]);
			}
		}
		this.#updateChildNodes(childNodes);
	}
