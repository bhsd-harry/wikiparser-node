 ...
 */
/** @typedef {import('./node').AstListener} AstListener */
...
class AstNode {
	...
	getAttribute(key) {
		if (key === 'optional') {
			return /** @type {TokenAttribute<T>} */ (new Set(this.#optional));
		} else if (key === 'verifyChild') {
			return /** @type {TokenAttribute<T>} */ (this.#verifyChild);
		}
	...
	setAttribute(key, value = undefined) {
		if (key === 'parentNode') {
			...
			const oldValue = this[/** @type {string} */ (key)],
	 ...
	 * 插入子节点
	 * @this {this & Token}
	 * @param {AstText|Token} node 待插入的子节点
	 * @param {number} i 插入位置
	 * @complexity `n`
	...
	insertAt(node, i = this.childNodes.length) {
		if (!(node instanceof AstNode)) {
			this.typeError('insertAt', 'AstNode');
	 ...
	 * @template {AstText|Token} T
	 * @param {T} child 插入节点
	 * @param {AstText|Token} reference 指定位置处的子节点
	 * @complexity `n`
	 */
	 ...
	 * @this {this & Token}
	 * @template {AstText|Token} T
	 * @param {AstText|Token} newChild 新子节点
	 ...
	 * 在当前节点前后插入兄弟节点
	 * @this {this & (AstText|Token)}
	 * @param {(AstText|Token)[]} nodes 插入节点
	 ...
	 * 是自身或后代节点
	 * @this {this & (AstText|Token)}
	 * @param {AstText|Token} node 待检测节点
	 * @returns {boolean}
	 * @complexity `n`
	 */
	contains(node) {
	 ...
	 */
	addEventListener(types, listener, options = undefined) {
	...
	listEventListeners(type) {
		return typeof type === 'string'
			? /** @type {AstListener[]} */ (this.#events.listeners(type))
	 ...
	 * 比较和另一个节点的相对位置
	 * @this {this & (AstText|Token)}
	 * @param {AstText|Token} other 待比较的节点
	 * @complexity `n`
	 ...
	 * @this {this & (AstText|Token)}
	 * @param {number} j 子节点序号
	 * @complexity `n`
	 */
	#getPosition(j = undefined) {
		return j === undefined
