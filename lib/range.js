...
class Ranges {
	...
	/** @param {number|string|Range|Iterable<number|string|Range>} arr */
	constructor(arr) {
		if (arr === undefined) {
			return;
		}
		arr = typeof arr !== 'string' && arr[Symbol.iterator] ? [...arr] : [arr];
		for (const ele of arr) {
			if (ele instanceof Range) {
				this.#array.push(new Range(ele));
				continue;
			}
