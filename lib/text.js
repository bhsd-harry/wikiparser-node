	...
	lint(start = 0) {
		...
		if (errors.length > 0) {
			...
			return errors.map(/** @returns {LintError} */ ({0: error, 1: prefix, 2: tag, index}) => {
				...
				const isAttr = parentNode?.type === 'attr-value',
					attr = parentNode?.parentNode,
					attrName = attr?.name,
					attrParent = attr?.parentNode?.name;
				if (isAttr && (attrName === 'before' || attrName === 'after') && attrParent === 'choose') {
					return false;
				} else if (char === 'h') {
					switch (parentNode?.type) {
						case 'ext-link-text':
							...
							break;
						case 'attr-value':
							if ((attrName === 'src' || attrName === 'srcset') && attrParent === 'img') {
								return false;
							}
							break;
						case 'ext-inner': {
					...
					&& isAttr && attrName === 'caption' && attrParent === 'gallery'
				) {
					return false;
				}
				return {
