...
class Text extends AstNode {
	...
	appendData(text) {
		...
		this.replaceData(this.data + text);
	}
	...
	deleteData(offset, count) {
		...
		this.replaceData(this.data.slice(0, offset) + this.data.slice(offset + count));
	}
	...
	insertData(offset, text) {
		...
		this.replaceData(this.data.slice(0, offset) + text + this.data.slice(offset));
	}
	...
	splitText(offset) {
		...
		this.replaceData(this.data.slice(0, offset));
		this.after(newText);
		return newText;
	}
}
