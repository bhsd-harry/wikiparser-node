	...
	lint(start = 0) {
		...
		if (data.search(errorRegex) !== -1) {
			...
			const root = this.getRootNode(),
				...
				tags = new Set([this.#config.ext, this.#config.html].flat(2));
			for (let mt = errorRegex.exec(data); mt; mt = errorRegex.exec(data)) {
				...
				if (char === 'h') {
					...
					const {constructor: Element} = document.createElement(tag);
					if (Element === HTMLElement || Element === HTMLUnknownElement) {
						continue;
					}
				}
				errors.push({
