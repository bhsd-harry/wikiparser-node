...
class ReferencesToken extends Token {
	...
	insertAt(token, i = this.childNodes.length) {
		return token.type === 'ext' && token.name !== 'ref'
			? this.typeError(`${this.constructor.name}只能以<ref>标签作为子节点！`)
			: super.insertAt(token, i);
	}

	/** @override */
	cloneNode() {
		const cloned = this.lastChild.cloneNode(),
			config = this.getAttribute('config'),
			token = Parser.run(() => new ReferencesToken(undefined, config));
